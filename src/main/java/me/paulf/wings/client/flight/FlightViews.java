...
public final class FlightViews {
    ...
    private static final CapabilityHolder<LivingEntity, FlightView, CapabilityHolder.State<LivingEntity, FlightView>> HOLDER =
            CapabilityHolder.create();

    public static final Capability<FlightView> CAPABILITY =
    ...
    public static void onAttachCapabilities(AttachFlightCapabilityEvent event) {
        ...
        if (entity instanceof AbstractClientPlayer) {
            event.addCapability(
                    ...
                    HOLDER.state().providerBuilder(
                            new FlightViewDefault((Player) entity, event.getInstance())
                    ).build()
            );
        }
    }
