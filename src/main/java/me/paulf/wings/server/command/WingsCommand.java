...
public class WingsCommand {
    ...
    private static int giveWing(CommandContext<CommandSourceStack> ctx) throws CommandSyntaxException {
        ...
        if (targets.size() == 1) {
            ctx.getSource().sendSuccess(() ->
                    Component.translatable("commands.wings.give.success.single", targets.iterator().next().getDisplayName()), true);
        } else {
            ctx.getSource().sendSuccess(() ->
                    Component.translatable("commands.wings.give.success.multiple", targets.size()), true);
        }
        return count;
    }
    ...
    private static int takeWings(CommandContext<CommandSourceStack> ctx) throws CommandSyntaxException {
        ...
        if (targets.size() == 1) {
            ctx.getSource().sendSuccess(() ->
                    Component.translatable("commands.wings.take.success.single", targets.iterator().next().getDisplayName()), true);
        } else {
            ctx.getSource().sendSuccess(() ->
                    Component.translatable("commands.wings.take.success.multiple", targets.size()), true);
        }
        return count;
    }
    ...
    private static int takeSpecificWings(CommandContext<CommandSourceStack> ctx) throws CommandSyntaxException {
        ...
        int count = 0;
        for (ServerPlayer player : targets) {
            if (BatBloodBottleItem.removeSpecificWings(player, wings)) {
                count++;
            }
        }
        if (count == 0) {
